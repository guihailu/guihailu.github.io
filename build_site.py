...
body_html = re.sub(
    r'<blockquote>\s*(<p>💜 <strong>归海2026.*?</p>)\s*</blockquote>',
    r'<div class="intro">\1</div>', body_html, count=1, flags=re.S)
...
.doctitle .seal{display:inline-block;margin-left:14px;vertical-align:8px;width:38px;height:54px;border-radius:8px;background:linear-gradient(160deg,#c24430,#93291b);color:#fff5e0;font-size:20px;line-height:27px;text-align:center;padding-top:0;letter-spacing:0;box-shadow:0 2px 8px rgba(0,0,0,.35);font-family:"KaiTi","STKaiti",serif;}
.doctitle .sub{margin:4px 0 0;color:var(--ink2);font-size:15px;letter-spacing:.26em;}
.intro{max-width:760px;margin:30px auto 36px;text-align:left;font-size:15px;color:var(--ink);background:var(--paper2);border-left:3px solid var(--cin);border-radius:10px;padding:16px 22px;line-height:1.9;}
.intro p{margin:0;}
